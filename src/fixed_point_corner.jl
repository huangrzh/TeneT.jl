...
# Left and Right environment: L2U rotation (R to down)
function obs_env_LR(M::AbstractArray; χ::Int, tol::Real=1e-10, maxiter::Int=10, miniter::Int=1, verbose=false, savefile= false, infolder::String="./data_lr/", outfolder::String="./data_lr/", updown = true, downfromup = false, show_every = Inf, savetol = 1e-5)
    Mlr = permutedims(M, (2,3,4,1,6,5))
    env_lr = obs_env(Mlr; χ=χ, tol=tol, maxiter=maxiter, miniter=miniter, verbose=verbose, 
    savefile=savefile, infolder=infolder, 
    outfolder=outfolder, updown=updown, 
    downfromup=downfromup, show_every=show_every, savetol=savetol)
    return env_lr
end
